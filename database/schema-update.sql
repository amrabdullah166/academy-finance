-- نظام إدارة الأكاديمية المالي - تحديث قاعدة البيانات
-- Database Schema Update for Academy Financial Management System

-- حذف الجداول الموجودة إذا كانت موجودة (بالترتيب العكسي للتبعيات)
DROP TABLE IF EXISTS audit_log CASCADE;
DROP TABLE IF EXISTS activities CASCADE;
DROP TABLE IF EXISTS monthly_subscriptions CASCADE;
...
-- حذف الـ Views
DROP VIEW IF EXISTS student_payment_summary CASCADE;
DROP VIEW IF EXISTS course_analytics CASCADE;
DROP VIEW IF EXISTS recent_activities CASCADE;
DROP VIEW IF EXISTS overdue_subscriptions CASCADE;
DROP VIEW IF EXISTS monthly_expenses_summary CASCADE;
DROP VIEW IF EXISTS monthly_revenue_summary CASCADE;

-- حذف الدوال والتريجرز
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;
DROP FUNCTION IF EXISTS generate_receipt_number() CASCADE;
DROP FUNCTION IF EXISTS create_monthly_subscription() CASCADE;
DROP FUNCTION IF EXISTS get_financial_stats(DATE, DATE) CASCADE;
DROP FUNCTION IF EXISTS log_activity() CASCADE;
DROP FUNCTION IF EXISTS update_overdue_subscriptions() CASCADE;
DROP FUNCTION IF EXISTS get_dashboard_quick_stats() CASCADE;

-- حذف التسلسل
DROP SEQUENCE IF EXISTS receipt_sequence CASCADE;

-- الآن إنشاء كل شيء من جديد
-- 1. جدول الموظفين (Employees) - أولاً لأنه مرجع في جداول أخرى
...
BEGIN
    ...
    IF TG_TABLE_NAME = 'payments' AND TG_OP = 'INSERT' THEN
        ...
        VALUES (
            ...
            'تم تسجيل دفعة جديدة رقم ' || COALESCE(NEW.receipt_number, 'غير محدد'),
            'payments',
            NEW.id,
            NEW.amount
        );
...
CREATE TRIGGER log_course_activity AFTER INSERT ON courses FOR EACH ROW EXECUTE FUNCTION log_activity();
...
SELECT 
    ...
    CASE 
        ...
        WHEN a.entity_type = 'payments' THEN CONCAT('إيصال رقم: ', COALESCE(p.receipt_number, 'غير محدد'))
